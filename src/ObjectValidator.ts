import type { ValidationContext, ValidationFields, ValidationMessage, ValidationSettings } from './types';
...
function resolveMessage(message : ValidationMessage, context : ValidationContext) {
...
export class ObjectValidator {
    ...
    private async evaluateFields( {
        ...
        for (let field of fields) {
            ...
            if (!field.isOptional && isEmpty) {
                invalidate(path, 
                    resolveMessage(field.emptyFieldMessage || this.mandatoryFieldError, aContext));
                continue;
            }
            ...
            if (Array.isArray(get(source, field.name)) && field.fields) {
                ...
                for (const subData of get(source, field.name)) {
                    valid = await this.evaluateFields({
                        fields: field.fields,
                        source: subData,
                        out,
                        contextData,
            ...
            for (let test of field.tests) {
                if (!(await test.fn(fieldData, aContext))) {
                    const message = test.message ? resolveMessage(test.message, aContext) : DEFAULT_FAILED_FIELD_MESSAGE;
    ...
    async validate({stateObj, source, contextData} : ValidationSettings ) {
        ...
        const out = Object.keys(stateObj.fields)
